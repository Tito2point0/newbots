...
const goToCart = require('./actions/goToCart');
...
async function init() {
  ...
  try {
    ...
    const cartStatus = await goToCart(page);
    console.log("👜 goToCart() result:", cartStatus);
    ...
    await db('stock_logs').insert({
      ...
      message: `addToBag: ${addStatus}, cart: ${cartStatus}, checkbox: ${checkboxStatus}`,
      http_status: status,
    ...
    // Do not close browser if debugging
    // await browser.close();
